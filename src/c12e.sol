...
contract Entity is Owned {  
    ...
    event AddedAttribute(
        uint indexed attribute_id, 
        string attribute_type, 
        string attribute_data); 
        
    event SignedAttribute(
        uint indexed certificate_id, 
        address indexed signer, 
        uint indexed attribute_id, 
        uint timestamp_validity_end); 
        
    event RevokedSignature(
        uint indexed revocation_id, 
        uint indexed certificate_id); 
        
    function setAttribute(string attribute_type, string attribute_data ) 
    onlyOwner{   
    ...
    function signAttribute(uint attribute_id, uint timestamp_validity_end )   
    onlyGrantedAddress { 
    ...
    function revokeSignature(uint certificate_id) { 
        if (certificates[certificate_id].signer == msg.sender) { 
    ...
    function getAttributeEveryOne(uint attribute_id)    
    constant returns (string, string, address) {
